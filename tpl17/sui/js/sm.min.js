...
+ function(a) {
	...
	a.smConfig = a.extend(b, a.config)
		...
		return b.statusBar ? k.push("with-statusbar-overlay") : a("html").removeClass("with-statusbar-overlay"), k.length > 0 && a("html").addClass(k.join(" ")), b
	}(Zepto), + function(a) {
		"use strict";
		...
		["width", "height"].forEach(function(b) {
			...
			if ("undefined" == typeof b) {
				if (this[0] && this[0].getAttribute) {
					...
					return c ? c : this[0].smElementDataStorage && a in this[0].smElementDataStorage ? this[0].smElementDataStorage[a] : void 0
				}
				return void 0
			}
			for (var d = 0; d < this.length; d++) {
				var e = this[d];
				e.smElementDataStorage || (e.smElementDataStorage = {}), e.smElementDataStorage[a] = b
			}
			return this
	...
	function() {
		...
		a.prototype.needsClick = function(a) {
			for (var b = a; b && "BODY" !== b.tagName.toUpperCase();) {
				if ("LABEL" === b.tagName.toUpperCase()) return !0;
				b = b.parentNode
			}
		...
		var b = function(b, c, d) {
				...
				if (e.addClass("active"), e.trigger("show"), c ? c = a(c) : (c = a("string" == typeof b ? '.tab-link[href="' + b + '"]' : '.tab-link[href="#' + e.attr("id") + '"]'), (!c || c && 0 === c.length) && a("[data-tab]").each(function() {
					...
					return c && c.length > 0 && c.addClass("active"), h && h.length > 0 && h.removeClass("active"), !0
				}
			},
			c = a.showTab;
		...
		a.modalStackClearQueue = function() {
			...
			return "function" == typeof c && (d = arguments[1], c = void 0), a.modal({
				text: b || "",
			...
			return b = a(b), 0 === b.length ? !1 : (b.show(), b.find(".content").scroller("refresh"), b.find("." + e.viewClass).length > 0 && a.sizeNavbars(b.find("." + e.viewClass)[0]), a.openModal(b), b[0])
			...
			var c = a("<div class='modal toast'>" + b + "</div>").appendTo(document.body);
			a.openModal(c), setTimeout(function() {
				a.closeModal(c)
			}, 2e3)
		...
		}, a(document).on("click", " .modal-overlay, .popup-overlay, .close-popup, .open-popup, .open-popover,  .close-picker", b);
		...
		var b = !1,
			c = function(c) {
				function d() {
					var b = !1;
					return j.params.convertToPopover || j.params.onlyInPopover ? (!j.inline && j.params.input && (j.params.onlyInPopover ? b = !0 : a.device.ios ? b = a.device.ipad ? !0 : !1 : a(window).width() >= 768 && (b = !0)), b) : b
				}

				function e() {
					return j.opened && j.container && j.container.length > 0 && j.container.parents(".popover").length > 0 ? !0 : !1
				}
				...
				function f(a) {
					...
					var b = a.getFullYear(),
						...
						e = a.getDate(),
						f = a.getDay();
					return j.params.dateFormat.replace(/yyyy/g, b).replace(/yy/g, (b + "").substring(2)).replace(/mm/g, 10 > d ? "0" + d : d).replace(/m/g, d).replace(/MM/g, j.params.monthNames[c]).replace(/M/g, j.params.monthNamesShort[c]).replace(/dd/g, 10 > e ? "0" + e : e).replace(/d/g, e).replace(/DD/g, j.params.dayNames[f]).replace(/D/g, j.params.dayNamesShort[f])
				}

				function g(a) {
					if (a.preventDefault(), !j.opened && (j.open(), j.params.scrollToInput && !d())) {
						var b = j.input.parents(".page-content");
						if (0 === b.length) return;
						var c, e = parseInt(b.css("padding-top"), 10),
							f = parseInt(b.css("padding-bottom"), 10),
							g = b[0].offsetHeight - e - j.container.height(),
							h = b[0].scrollHeight - e - j.container.height(),
							i = j.input.offset().top - e + j.input[0].offsetHeight;
						if (i > g) {
							var k = b.scrollTop() + i - g;
							k + g > h && (c = k + g - h + f, g === h && (c = j.container.height()), b.css({
								"padding-bottom": c + "px"
							})), b.scrollTop(k, 300)
						}
					}
				...
				function h(b) {
					e() || (j.input && j.input.length > 0 ? b.target !== j.input[0] && 0 === a(b.target).parents(".picker-modal").length && j.close() : 0 === a(b.target).parents(".picker-modal").length && j.close())
				}

				function i() {
					j.opened = !1, j.input && j.input.length > 0 && j.input.parents(".page-content").css({
						"padding-bottom": ""
					}), j.params.onClose && j.params.onClose(j), j.destroyCalendarEvents()
				}
				var j = this,
					k = {
				...
				c = c || {};
				for (var l in k) "undefined" == typeof c[l] && (c[l] = k[l]);
				j.params = c, j.initialized = !1, j.inline = j.params.container ? !0 : !1, j.isH = "horizontal" === j.params.direction;
				var m = j.isH && b ? -1 : 1;
				return j.animating = !1, j.addValue = function(a) {
					if (j.params.multiple) {
						j.value || (j.value = []);
						for (var b, c = 0; c < j.value.length; c++) new Date(a).getTime() === new Date(j.value[c]).getTime() && (b = c);
						"undefined" == typeof b ? j.value.push(a) : j.value.splice(b, 1), j.updateValue()
					} else j.value = [a], j.updateValue()
				}, j.setValue = function(a) {
					j.value = a, j.updateValue()
				}, j.updateValue = function() {
					j.wrapper.find(".picker-calendar-day-selected").removeClass("picker-calendar-day-selected");
					var b, c;
					for (b = 0; b < j.value.length; b++) {
						var d = new Date(j.value[b]);
						j.wrapper.find('.picker-calendar-day[data-date="' + d.getFullYear() + "-" + d.getMonth() + "-" + d.getDate() + '"]').addClass("picker-calendar-day-selected")
					}
					if (j.params.onChange && j.params.onChange(j, j.value, j.value.map(f)), j.input && j.input.length > 0) {
						if (j.params.formatValue) c = j.params.formatValue(j, j.value);
						else {
							for (c = [], b = 0; b < j.value.length; b++) c.push(f(j.value[b]));
							c = c.join(", ")
						}
						a(j.input).val(c), a(j.input).trigger("change")
					}
				}, j.initCalendarEvents = function() {
					function c(a) {
						h || g || (g = !0, i = n = "touchstart" === a.type ? a.targetTouches[0].pageX : a.pageX, k = n = "touchstart" === a.type ? a.targetTouches[0].pageY : a.pageY, o = (new Date).getTime(), u = 0, x = !0, w = void 0, q = r = j.monthsTranslate)
					}

					function d(a) {
						if (g) {
							if (l = "touchmove" === a.type ? a.targetTouches[0].pageX : a.pageX, n = "touchmove" === a.type ? a.targetTouches[0].pageY : a.pageY, "undefined" == typeof w && (w = !!(w || Math.abs(n - k) > Math.abs(l - i))), j.isH && w) return void(g = !1);
							if (a.preventDefault(), j.animating) return void(g = !1);
							x = !1, h || (h = !0, s = j.wrapper[0].offsetWidth, t = j.wrapper[0].offsetHeight, j.wrapper.transition(0)), a.preventDefault(), v = j.isH ? l - i : n - k, u = v / (j.isH ? s : t), r = 100 * (j.monthsTranslate * m + u), j.wrapper.transform("translate3d(" + (j.isH ? r : 0) + "%, " + (j.isH ? 0 : r) + "%, 0)")
						}
					}

					function e(a) {
						return g && h ? (g = h = !1, p = (new Date).getTime(), 300 > p - o ? Math.abs(v) < 10 ? j.resetMonth() : v >= 10 ? b ? j.nextMonth() : j.prevMonth() : b ? j.prevMonth() : j.nextMonth() : -.5 >= u ? b ? j.prevMonth() : j.nextMonth() : u >= .5 ? b ? j.nextMonth() : j.prevMonth() : j.resetMonth(), void setTimeout(function() {
							x = !0
						}, 100)) : void(g = h = !1)
					}
					...
					function f(b) {
						if (x) {
							...
							if (0 === c.length && a(b.target).hasClass("picker-calendar-day") && (c = a(b.target)), 0 !== c.length && (!c.hasClass("picker-calendar-day-selected") || j.params.multiple) && !c.hasClass("picker-calendar-day-disabled")) {
								c.hasClass("picker-calendar-day-next") && j.nextMonth(), c.hasClass("picker-calendar-day-prev") && j.prevMonth();
								var d = c.attr("data-year"),
									e = c.attr("data-month"),
									f = c.attr("data-day");
								j.params.onDayClick && j.params.onDayClick(j, c[0], d, e, f), j.addValue(new Date(d, e, f).getTime()), j.params.closeOnSelect && j.close()
							}
						}
					}
					var g, h, i, k, l, n, o, p, q, r, s, t, u, v, w, x = !0;
					j.container.find(".picker-calendar-prev-month").on("click", j.prevMonth), j.container.find(".picker-calendar-next-month").on("click", j.nextMonth), j.container.find(".picker-calendar-prev-year").on("click", j.prevYear), j.container.find(".picker-calendar-next-year").on("click", j.nextYear), j.wrapper.on("click", f), j.params.touchMove && (j.wrapper.on(a.touchEvents.start, c), j.wrapper.on(a.touchEvents.move, d), j.wrapper.on(a.touchEvents.end, e)), j.container[0].f7DestroyCalendarEvents = function() {
						j.container.find(".picker-calendar-prev-month").off("click", j.prevMonth), j.container.find(".picker-calendar-next-month").off("click", j.nextMonth), j.container.find(".picker-calendar-prev-year").off("click", j.prevYear), j.container.find(".picker-calendar-next-year").off("click", j.nextYear), j.wrapper.off("click", f), j.params.touchMove && (j.wrapper.off(a.touchEvents.start, c), j.wrapper.off(a.touchEvents.move, d), j.wrapper.off(a.touchEvents.end, e))
					}
				}, j.destroyCalendarEvents = function(a) {
					"f7DestroyCalendarEvents" in j.container[0] && j.container[0].f7DestroyCalendarEvents()
				}, j.daysInMonth = function(a) {
					var b = new Date(a);
					return new Date(b.getFullYear(), b.getMonth() + 1, 0).getDate()
				}, j.monthHTML = function(a, b) {
					a = new Date(a);
					var c = a.getFullYear(),
						d = a.getMonth();
					a.getDate();
					...
					var e = j.daysInMonth(new Date(a.getFullYear(), a.getMonth()).getTime() - 864e6),
						f = j.daysInMonth(a),
					...
					var h, i, k, l = [],
						...
						n = 7,
						o = "",
						p = 0 + (j.params.firstDay - 1),
						q = (new Date).setHours(0, 0, 0, 0),
						r = j.params.minDate ? new Date(j.params.minDate).getTime() : null,
						s = j.params.maxDate ? new Date(j.params.maxDate).getTime() : null;
					if (j.value && j.value.length)
						for (i = 0; i < j.value.length; i++) l.push(new Date(j.value[i]).setHours(0, 0, 0, 0));
					for (i = 1; m >= i; i++) {
						var t = "";
						for (k = 1; n >= k; k++) {
							var u = k;
				...
				}, j.animating = !1, j.updateCurrentMonthYear = function(a) {
					"undefined" == typeof a ? (j.currentMonth = parseInt(j.months.eq(1).attr("data-month"), 10), j.currentYear = parseInt(j.months.eq(1).attr("data-year"), 10)) : (j.currentMonth = parseInt(j.months.eq("next" === a ? j.months.length - 1 : 0).attr("data-month"), 10), j.currentYear = parseInt(j.months.eq("next" === a ? j.months.length - 1 : 0).attr("data-year"), 10)), j.container.find(".current-month-value").text(j.params.monthNames[j.currentMonth]), j.container.find(".current-year-value").text(j.currentYear)
				}, j.onMonthChangeStart = function(a) {
					j.updateCurrentMonthYear(a), j.months.removeClass("picker-calendar-month-current picker-calendar-month-prev picker-calendar-month-next");
					var b = "next" === a ? j.months.length - 1 : 0;
					j.months.eq(b).addClass("picker-calendar-month-current"), j.months.eq("next" === a ? b - 1 : b + 1).addClass("next" === a ? "picker-calendar-month-prev" : "picker-calendar-month-next"), j.params.onMonthYearChangeStart && j.params.onMonthYearChangeStart(j, j.currentYear, j.currentMonth)
				}, j.onMonthChangeEnd = function(a, b) {
					j.animating = !1;
					var c, d, e;
					j.wrapper.find(".picker-calendar-month:not(.picker-calendar-month-prev):not(.picker-calendar-month-current):not(.picker-calendar-month-next)").remove(), "undefined" == typeof a && (a = "next", b = !0), b ? (j.wrapper.find(".picker-calendar-month-next, .picker-calendar-month-prev").remove(), d = j.monthHTML(new Date(j.currentYear, j.currentMonth), "prev"), c = j.monthHTML(new Date(j.currentYear, j.currentMonth), "next")) : e = j.monthHTML(new Date(j.currentYear, j.currentMonth), a), ("next" === a || b) && j.wrapper.append(e || c), ("prev" === a || b) && j.wrapper.prepend(e || d), j.months = j.wrapper.find(".picker-calendar-month"), j.setMonthsTranslate(j.monthsTranslate), j.params.onMonthAdd && j.params.onMonthAdd(j, "next" === a ? j.months.eq(j.months.length - 1)[0] : j.months.eq(0)[0]), j.params.onMonthYearChangeEnd && j.params.onMonthYearChangeEnd(j, j.currentYear, j.currentMonth)
				}, j.setMonthsTranslate = function(a) {
					a = a || j.monthsTranslate || 0, "undefined" == typeof j.monthsTranslate && (j.monthsTranslate = a), j.months.removeClass("picker-calendar-month-current picker-calendar-month-prev picker-calendar-month-next");
					var b = 100 * -(a + 1) * m,
						c = 100 * -a * m,
						d = 100 * -(a - 1) * m;
					j.months.eq(0).transform("translate3d(" + (j.isH ? b : 0) + "%, " + (j.isH ? 0 : b) + "%, 0)").addClass("picker-calendar-month-prev"), j.months.eq(1).transform("translate3d(" + (j.isH ? c : 0) + "%, " + (j.isH ? 0 : c) + "%, 0)").addClass("picker-calendar-month-current"), j.months.eq(2).transform("translate3d(" + (j.isH ? d : 0) + "%, " + (j.isH ? 0 : d) + "%, 0)").addClass("picker-calendar-month-next")
				}, j.nextMonth = function(b) {
					("undefined" == typeof b || "object" == typeof b) && (b = "", j.params.animate || (b = 0));
					var c = parseInt(j.months.eq(j.months.length - 1).attr("data-month"), 10),
						d = parseInt(j.months.eq(j.months.length - 1).attr("data-year"), 10),
						e = new Date(d, c),
						f = e.getTime(),
						g = j.animating ? !1 : !0;
					if (j.params.maxDate && f > new Date(j.params.maxDate).getTime()) return j.resetMonth();
					if (j.monthsTranslate--, c === j.currentMonth) {
						var h = 100 * -j.monthsTranslate * m,
							i = a(j.monthHTML(f, "next")).transform("translate3d(" + (j.isH ? h : 0) + "%, " + (j.isH ? 0 : h) + "%, 0)").addClass("picker-calendar-month-next");
						j.wrapper.append(i[0]), j.months = j.wrapper.find(".picker-calendar-month"), j.params.onMonthAdd && j.params.onMonthAdd(j, j.months.eq(j.months.length - 1)[0])
					}
					j.animating = !0, j.onMonthChangeStart("next");
					var k = 100 * j.monthsTranslate * m;
					j.wrapper.transition(b).transform("translate3d(" + (j.isH ? k : 0) + "%, " + (j.isH ? 0 : k) + "%, 0)"), g && j.wrapper.transitionEnd(function() {
						j.onMonthChangeEnd("next")
					}), j.params.animate || j.onMonthChangeEnd("next")
				}, j.prevMonth = function(b) {
					("undefined" == typeof b || "object" == typeof b) && (b = "", j.params.animate || (b = 0));
					var c = parseInt(j.months.eq(0).attr("data-month"), 10),
						d = parseInt(j.months.eq(0).attr("data-year"), 10),
						e = new Date(d, c + 1, -1),
						f = e.getTime(),
						g = j.animating ? !1 : !0;
					if (j.params.minDate && f < new Date(j.params.minDate).getTime()) return j.resetMonth();
					if (j.monthsTranslate++, c === j.currentMonth) {
						var h = 100 * -j.monthsTranslate * m,
							i = a(j.monthHTML(f, "prev")).transform("translate3d(" + (j.isH ? h : 0) + "%, " + (j.isH ? 0 : h) + "%, 0)").addClass("picker-calendar-month-prev");
						j.wrapper.prepend(i[0]), j.months = j.wrapper.find(".picker-calendar-month"), j.params.onMonthAdd && j.params.onMonthAdd(j, j.months.eq(0)[0])
					}
					j.animating = !0, j.onMonthChangeStart("prev");
					var k = 100 * j.monthsTranslate * m;
					j.wrapper.transition(b).transform("translate3d(" + (j.isH ? k : 0) + "%, " + (j.isH ? 0 : k) + "%, 0)"), g && j.wrapper.transitionEnd(function() {
						j.onMonthChangeEnd("prev")
					}), j.params.animate || j.onMonthChangeEnd("prev")
				}, j.resetMonth = function(a) {
					"undefined" == typeof a && (a = "");
					var b = 100 * j.monthsTranslate * m;
					j.wrapper.transition(a).transform("translate3d(" + (j.isH ? b : 0) + "%, " + (j.isH ? 0 : b) + "%, 0)")
				}, j.setYearMonth = function(a, b, c) {
					"undefined" == typeof a && (a = j.currentYear), "undefined" == typeof b && (b = j.currentMonth), ("undefined" == typeof c || "object" == typeof c) && (c = "", j.params.animate || (c = 0));
					var d;
					if (d = a < j.currentYear ? new Date(a, b + 1, -1).getTime() : new Date(a, b).getTime(), j.params.maxDate && d > new Date(j.params.maxDate).getTime()) return !1;
					if (j.params.minDate && d < new Date(j.params.minDate).getTime()) return !1;
					var e = new Date(j.currentYear, j.currentMonth).getTime(),
						f = d > e ? "next" : "prev",
						g = j.monthHTML(new Date(a, b));
					j.monthsTranslate = j.monthsTranslate || 0;
					var h, i, k = j.monthsTranslate,
						l = j.animating ? !1 : !0;
					d > e ? (j.monthsTranslate--, j.animating || j.months.eq(j.months.length - 1).remove(), j.wrapper.append(g), j.months = j.wrapper.find(".picker-calendar-month"), h = 100 * -(k - 1) * m, j.months.eq(j.months.length - 1).transform("translate3d(" + (j.isH ? h : 0) + "%, " + (j.isH ? 0 : h) + "%, 0)").addClass("picker-calendar-month-next")) : (j.monthsTranslate++, j.animating || j.months.eq(0).remove(), j.wrapper.prepend(g), j.months = j.wrapper.find(".picker-calendar-month"), h = 100 * -(k + 1) * m, j.months.eq(0).transform("translate3d(" + (j.isH ? h : 0) + "%, " + (j.isH ? 0 : h) + "%, 0)").addClass("picker-calendar-month-prev")), j.params.onMonthAdd && j.params.onMonthAdd(j, "next" === f ? j.months.eq(j.months.length - 1)[0] : j.months.eq(0)[0]), j.animating = !0, j.onMonthChangeStart(f), i = 100 * j.monthsTranslate * m, j.wrapper.transition(c).transform("translate3d(" + (j.isH ? i : 0) + "%, " + (j.isH ? 0 : i) + "%, 0)"), l && j.wrapper.transitionEnd(function() {
						j.onMonthChangeEnd(f, !0)
					}), j.params.animate || j.onMonthChangeEnd(f)
				}, j.nextYear = function() {
					j.setYearMonth(j.currentYear + 1)
				}, j.prevYear = function() {
					j.setYearMonth(j.currentYear - 1)
				}, j.layout = function() {
					var a, b = "",
						c = "",
						d = j.value && j.value.length ? j.value[0] : (new Date).setHours(0, 0, 0, 0),
						e = j.monthHTML(d, "prev"),
						f = j.monthHTML(d),
						g = j.monthHTML(d, "next"),
						h = '<div class="picker-calendar-months"><div class="picker-calendar-months-wrapper">' + (e + f + g) + "</div></div>",
						i = "";
					if (j.params.weekHeader) {
						for (a = 0; 7 > a; a++) {
							var k = a + j.params.firstDay > 6 ? a - 7 + j.params.firstDay : a + j.params.firstDay,
								l = j.params.dayNamesShort[k];
							i += '<div class="picker-calendar-week-day ' + (j.params.weekendDays.indexOf(k) >= 0 ? "picker-calendar-week-day-weekend" : "") + '"> ' + l + "</div>"
						}
						i = '<div class="picker-calendar-week-days">' + i + "</div>"
					}
					c = "picker-modal picker-calendar " + (j.params.cssClass || "");
					var m = j.params.toolbar ? j.params.toolbarTemplate.replace(/{{closeText}}/g, j.params.toolbarCloseText) : "";
					j.params.toolbar && (m = j.params.toolbarTemplate.replace(/{{closeText}}/g, j.params.toolbarCloseText).replace(/{{monthPicker}}/g, j.params.monthPicker ? j.params.monthPickerTemplate : "").replace(/{{yearPicker}}/g, j.params.yearPicker ? j.params.yearPickerTemplate : "")), b = '<div class="' + c + '">' + m + '<div class="picker-modal-inner">' + i + h + "</div></div>", j.pickerHTML = b
				}, j.params.input && (j.input = a(j.params.input), j.input.length > 0 && (j.params.inputReadOnly && j.input.prop("readOnly", !0), j.inline || j.input.on("click", g), j.params.inputReadOnly && j.input.on("focus mousedown", function(a) {
					a.preventDefault()
				}))), j.inline || a("html").on("click", h), j.opened = !1, j.open = function() {
					var b = d(),
						c = !1;
					j.opened || (j.value || j.params.value && (j.value = j.params.value, c = !0), j.layout(), b ? (j.pickerHTML = '<div class="popover popover-picker-calendar"><div class="popover-inner">' + j.pickerHTML + "</div></div>", j.popover = a.popover(j.pickerHTML, j.params.input, !0), j.container = a(j.popover).find(".picker-modal"), a(j.popover).on("close", function() {
						i()
					})) : j.inline ? (j.container = a(j.pickerHTML), j.container.addClass("picker-modal-inline"), a(j.params.container).append(j.container)) : (j.container = a(a.pickerModal(j.pickerHTML)), a(j.container).on("close", function() {
						i()
					})), j.container[0].f7Calendar = j, j.wrapper = j.container.find(".picker-calendar-months-wrapper"), j.months = j.wrapper.find(".picker-calendar-month"), j.updateCurrentMonthYear(), j.monthsTranslate = 0, j.setMonthsTranslate(), j.initCalendarEvents(), c && j.updateValue()), j.opened = !0, j.initialized = !0, j.params.onMonthAdd && j.months.each(function() {
						j.params.onMonthAdd(j, this)
					}), j.params.onOpen && j.params.onOpen(j)
				}, j.close = function() {
					return j.opened && !j.inline ? e() ? void a.closeModal(j.popover) : void a.closeModal(j.container) : void 0
				}, j.destroy = function() {
					j.close(), j.params.input && j.input.length > 0 && j.input.off("click focus", g), a("html").off("click", h)
				}, j.inline && j.open(), j
			};
		...
		var b = function(b) {
			...
			function f(a) {
				if (a.preventDefault(), !i.opened && (i.open(), i.params.scrollToInput && !c())) {
					var b = i.input.parents(".page-content");
					if (0 === b.length) return;
			...
			i.params = b, i.cols = [], i.initialized = !1, i.inline = i.params.container ? !0 : !1;
			...
			return i.setValue = function(a, b) {
				...
				function f(b) {
					if (t) {
						...
						var c = w - v;
						B = z + c, A = void 0, q > B && (B = q - Math.pow(q - B, .8), A = "min"), B > r && (B = r + Math.pow(B - r, .8), A = "max"), m.wrapper.transform("translate3d(0," + B + "px,0)"), m.updateItems(void 0, B, 0, i.params.updateValuesOnTouchmove), D = B - C || B, E = (new Date).getTime(), C = B
					}
				}
			...
			}, i.params.input && (i.input = a(i.params.input), i.input.length > 0 && (i.params.inputReadOnly && i.input.prop("readOnly", !0), i.inline || i.input.on("click", f), i.params.inputReadOnly && i.input.on("focus mousedown", function(a) {
				a.preventDefault()
		...
		var b = new Date,
			...
			h = {
				rotateEffect: !1,
				value: [b.getFullYear(), e(b.getMonth() + 1), b.getDate(), b.getHours(), e(b.getMinutes())],
		...
		function b(b) {
			...
			return this.each(function() {
				var f = a(this),
					...
					h = f.data("scroller");
				return h || f.data("scroller", h = new e(this, g)), "string" == typeof b && "function" == typeof h[b] && (d = h[b].apply(h, c),
					void 0 !== d) ? !1 : void 0
			}), void 0 !== d ? d : this
		}
		var c = function(a, b) {
		...
		! function() {
			a.extend(a.fn, {
				scrollTop: function(a, b) {
					if (this.length) {
						var c = this.data("scroller");
						return c && c.scroller ? c.scrollTop(a, b) : d.scrollTop.apply(this, arguments)
					}
				}
			}), a.extend(a.fn, {
				scrollLeft: function(a, b) {
					if (this.length) {
						var c = this.data("scroller");
						return c && c.scroller ? c.scrollLeft(a, b) : d.scrollLeft.apply(this, arguments)
					}
				}
			})
		}();
		var e = function(b, d) {
			var e = this.$pageContent = a(b);
			this.options = a.extend({}, this._defaults, d);
			var f = this.options.type,
				g = "js" === f || "auto" === f && a.os.android && c("4.4.0", a.os.version) > -1 || a.os.ios && c("6.0.0", a.os.version) > -1;
			if (g) {
				var h = e.find(".content-inner");
				if (!h[0]) {
					var i = e.children();
					i.length < 1 ? e.children().wrapAll('<div class="content-inner"></div>') : e.html('<div class="content-inner">' + e.html() + "</div>")
				}
				e.hasClass("pull-to-refresh-content") && e.find(".content-inner").css("min-height", a(window).height() + 20 + "px");
				var j = a(b).hasClass("pull-to-refresh-content"),
					k = {
						probeType: 1,
						mouseWheel: !0
					};
				j && (k.ptr = !0, k.ptrOffset = 44), this.scroller = new IScroll(b, k), this._bindEventToDomWhenJs(), a.initPullToRefresh = a._pullToRefreshJSScroll.initPullToRefresh, a.pullToRefreshDone = a._pullToRefreshJSScroll.pullToRefreshDone, a.pullToRefreshTrigger = a._pullToRefreshJSScroll.pullToRefreshTrigger, a.destroyToRefresh = a._pullToRefreshJSScroll.destroyToRefresh, e.addClass("javascript-scroll");
				var l = this.$pageContent[0].scrollTop;
				l && (this.$pageContent[0].scrollTop = 0, this.scrollTop(l))
			} else e.addClass("native-scroll")
		};
		e.prototype = {
			_defaults: {
				type: "native"
		...
		var f = a.fn.scroller;
		a.fn.scroller = b, a.fn.scroller.Constructor = e, a.fn.scroller.noConflict = function() {
			return a.fn.scroller = f, this
		}, a(function() {
		...
		var b = 0,
			c = function(c) {
				...
				function e() {
					j.hasClass("refreshing") || (j.removeClass("pull-down pull-up"), j.addClass("refreshing transitioning"), j.trigger("refresh", {
						done: function() {
			...
			function d(b) {
				if (h) {
					...
					if (o = m[0].scrollTop, "undefined" == typeof p && 0 !== o && (p = !0), !i) {
						...
						r && (q = m.attr("data-ptr-distance"), q.indexOf("%") >= 0 && (q = m[0].offsetHeight * parseInt(q, 10) / 100)), v = m.hasClass("refreshing") ? q : 0, u = m[0].scrollHeight !== m[0].offsetHeight && a.os.ios ? !1 : !0, u = !0
					}
					return i = !0, k = d - s.y, k > 0 && 0 >= o || 0 > o ? (a.os.ios && parseInt(a.os.version.split(".")[0], 10) > 7 && 0 === o && !p && (u = !0), u && (b.preventDefault(), n = Math.pow(k, .85) + v, m.transform("translate3d(0," + n + "px,0)")), u && Math.pow(k, .85) > q || !u && k >= 2 * q ? (t = !0, m.addClass("pull-up").removeClass("pull-down")) : (t = !1, m.removeClass("pull-up").addClass("pull-down")), void 0) : (m.removeClass("pull-up pull-down"), void(t = !1))
				}
			}
		...
		var b = function() {
			this.state = sessionStorage, this.state.setItem("stateid", parseInt(this.state.getItem("stateid") || 1) + 1), this.state.setItem("currentStateID", this.state.getItem("stateid")), this.stack = sessionStorage, this.stack.setItem("back", "[]"), this.stack.setItem("forward", "[]"), this.init(), this.xhr = null
		};
		b.prototype.defaults = {}, b.prototype.init = function() {
			...
			if (!e) {
				...
				this.pushState(location.href, f), this.pushBack({
					url: location.href,
					pageid: b[0].id,
					id: f
			...
			this.getPage(b, function(c) {
				...
				for (var e = JSON.parse(this.state.getItem("forward") || "[]"), f = 0; f < e.length; f++) a(e[f].pageid).each(function() {
					var b = a(this);
					b.data("page-remote") && b.remove()
				});
				this.state.setItem("forward", "[]"), c.insertAfter(a(".page")[0]), this.animatePages(this.getCurrentPage(), c);
				var g = this.genStateID();
				this.setCurrentStateID(g), this.pushState(b, g), this.forwardStack = []
			})
		}, b.prototype.animatePages = function(a, b, c) {
			var d = "page-left page-right page-current page-from-center-to-left page-from-center-to-right page-from-right-to-center page-from-left-to-center";
			c ? (a.trigger("pageAnimationStart", [b[0].id, b]), a.removeClass(d).addClass("page-from-left-to-center"), b.removeClass(d).addClass("page-from-center-to-right"), a.animationEnd(function() {
				a.removeClass(d).addClass("page-current"), a.trigger("pageAnimationEnd", [a[0].id, a]), a.trigger("pageReinit", [a[0].id, a])
			}), b.animationEnd(function() {
				b.removeClass(d)
			})) : (b.trigger("pageAnimationStart", [b[0].id, b]), a.removeClass(d).addClass("page-from-center-to-left"), b.removeClass(d).addClass("page-from-right-to-center"), a.animationEnd(function() {
				a.removeClass(d)
			}), b.animationEnd(function() {
				b.removeClass(d).addClass("page-current"), b.trigger("pageAnimationEnd", [b[0].id, b]), b.trigger("pageInitInternal", [b[0].id, b])
			}))
			...
			b.length ? history.back() : location.href = a
			...
			d[0] && (this.pushForward({
				...
				pageid: "#" + c[0].id,
				id: this.getCurrentStateID()
			}), this.animatePages(d, c, !0), this.setCurrentStateID(b.id))
			...
			d[0] && (this.pushBack({
				...
				pageid: "#" + c[0].id,
				id: this.getCurrentStateID()
			}), this.animatePages(c, d), this.setCurrentStateID(b.id))
			...
			this.dispatch("pageLoadStart"), this.xhr && this.xhr.readyState < 4 && (xhr.onreadystatechange = noop, xhr.abort(), this.dispatch("pageLoadCancel"));
			var d = this;
			this.xhr = a.ajax({
				url: b,
				success: a.proxy(function(a, b, d) {
					var e = this.parseXHR(d);
					e[0].id || (e[0].id = this.genRandomID()), e.data("page-remote", 1), c.apply(this, [e])
				}, this),
				error: function() {
			...
			var c = b.responseText,
				d = c.match(/<body[^>]*>([\s\S.]*)<\/body>/i)[1];
			d || (d = c), d = "<div>" + d + "</div>";
			var e = a(d);
			e.find(".popup, .panel, .panel-overlay").appendTo(document.body);
			var f = e.find(".page");
			return f[0] || (f = e.addClass("page")), f
